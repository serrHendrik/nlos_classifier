function [Eph, iono, corrSysTime] = RINEX_get_nav(file_nav, constellations)

% SYNTAX:
%   [Eph, iono, ggtoParam] = RINEX_get_nav(file_nav, constellations);
%
% INPUT:
...
while (isempty(header_end))
    ...
    % if the GPS constellation is enabled read and extract the time
    % system correction parameters
    if (constellations.GPS.enabled == 1)
        % Line that contains the correction to system time parameters
        % (GPS TO UTC)
        GPUT_found = ~isempty(findstr(lin,'GPUT'));
       
        if (GPUT_found)
            % Extract the 4 parameters
            data = textscan(lin(5:end),'%f%f%f%f%*[^\n]');
            if ~isempty(data)
                % Assign them to the output variable
                corrSysTime(1) = data{1};
                corrSysTime(2) = data{2};
                corrSysTime(3) = data{3};
                corrSysTime(4) = data{4};
            end
        end
        % Line that contains ionospheric corrections (GPS)
        iono_found = contains(lin,["ION ALPHA","GPSA"]);
        iono_found2 = contains(lin,["ION BETA","GPSB"]);
    end
    
    % if the Galileo constellation is enabled read and extract the GGTO
    % parameters
    if (constellations.Galileo.enabled == 1)
        % Line that contains the GGTO parameters (A0G, A1G, t0G, Week Number)
        gpga_found = contains(lin,["GPGA","GAGP"]); % in Rinex 3.04 it becomes GAGP, to be taken into account
        ...
        % Line that contains ionospheric corrections (GAL)
        iono_found = startsWith(lin,["GAL", " GAL"]);
    end
    
    % if the GLONASS constellation is enabled read and extract the time
    % system correction parameters
    if (constellations.GLONASS.enabled == 1)
        % Line that contains the correction to system time parameters
        % (GLONASS TO UTC)
        GLUT_found = contains(lin,["GLUT", "CORR TO SYSTEM TIME"]);
