function [Eph, iono, corrSysTime] = load_RINEX_nav(filename, constellations, flag_SP3, wait_dlg)
...
if (~flag_SP3)
    ...
    if (constellations.GPS.enabled || flag_mixed)
        if (exist(filename,'file'))
            ...
            [Eph_G, iono_G, corrSysTime] = RINEX_get_nav(filename, constellations);
        else
    ...
    if (constellations.GLONASS.enabled)
        if (exist(filename,'file'))
            ...
            [Eph_R, iono_R, corrSysTime] = RINEX_get_nav(filename, constellations);
        elseif (~flag_mixed)
...
%             [Eph_E, iono_E] = RINEX_get_nav([filename(1:end-1) 'l'], constellations);
            [Eph_E, iono_E, corrSysTime] = RINEX_get_nav([filename(1:end)], constellations);
%         elseif (~flag_mixed)
