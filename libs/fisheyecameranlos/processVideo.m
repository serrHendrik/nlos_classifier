function [videoFrameStore,pbMat,videoReadID] = processVideo(conf,vboTag,cameraImSel,commonTime)
...
%     lSegm           = NaN(VideoReaderTest.Height,VideoReaderTest.Width,length(videoIDTag));
end
...
for i = 1:length(videosToLoad)
    ...
    for j = 1:length(IDCurrVideo)
        ...
        if aviSyncTimeSel(IDCurrVideo(j)) ~= -0.001 && aviSyncTimeSel(IDCurrVideo(j)) < durationCurrVideo
            ...
            if(conf.getProbMat == 1)
%                 [pbFrame, ~]   = img_PbMap_Conf(videoFrameStore(:,:,:,IDCurrVideo(j)));
%                 fprintf(confFileID,'%d %f %f \n',j,aviSyncTimeSel(IDCurrVideo(j)),pbConf);
                [pbFrame]           = waterShedSegmentation(videoFrameStore(:,:,:,IDCurrVideo(j)),cameraMask);
                pbMat(:,:,j)        = pbFrame;
%                 lSegm(:,:,j)        = L;
            end
